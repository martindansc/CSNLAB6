...
versions <- c("BA", "BA_RA")
names(versions) <- c("Barabasi-Albert", "Random Attachment")
...
stat_function(fun=function(x) m0[v] * sqrt(x), geom="line")

v <- "BA_RA"
ggplot(degreeOverTime$BA_RA, aes(x = t, y = degree)) + 
geom_line(aes(color = as.factor(ti)), size = 1) +
theme_minimal() + labs(color = "Arrival time") + 
stat_function(fun=function(x) m0[v]*log(n0[v]+x-1), geom="line")

# v <- "BA_NG"
# ggplot(degreeOverTime$BA_NG, aes(x = t, y = degree)) + 
# geom_line(aes(color = as.factor(ti)), size = 1) +
# theme_minimal() + labs(color = "Arrival time")
