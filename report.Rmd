...
```{r}
kable(data.frame(Version = names(versions), m0 = m0, n0 = n0, InitialConf = c("Ring", "Ring", "Ring")), row.names = F)
```
...
In order to do the simulation we developed a in c++ a small program that implemented all the simulations. We defined a class named "representation" that store the degree of each vertex, the adjancy matrix and a vector of vertices where each vertex appears the same amount of times as it's degree, and with that, we could make fast simulations. 
...
When making the plots we realised that they have some variance due to the randomness, we would have liked to compute an average of progression of the vertices in order to have much better fits to the theoretical one, but we run out of time.
