...
for(v in versions){
  ...
  if(version == "BA") ts$degree <- ts$degree*sqrt(ts$ti)
  if(version == "BA_RA") ts$degree <- ts$degree + m0[v]*log(n0[v]+ts$ti-1)-m0[v]
...
# Scaled version of ki(t)
if(version == "BA") ts$degree <- ts$degree*sqrt(ts$ti)
if(version == "BA_RA") ts$degree <- ts$degree + m0*log(n0+ts$ti-1)-m0
...
solve1Version <- function(version){
  lapply(arrivalTimes, function(x) solveAllModels(degreeOverTime$BA[degreeOverTime[[version]]$ti==x,]))
}
```
...
theoreticalCurves <- list(
  BA = function(x) m0*sqrt(x),
  BA_RA = function(x) m0*log(m0+x-1),
  BA_NG = function(x) 2*m0*x/n0
)
...
plotEvolutions <- function(version){
  ggplot(degreeOverTime[[version]], aes(x = t, y = degree)) + 
    geom_line(aes(color = as.factor(ti)), size = 1) +
    theme_minimal() +
    stat_function(fun = theoreticalCurves[[version]], mapping = aes(color = "Theoretical")) +
...
nls_results <- lapply("BA", solve1Version)
```

## Evaluating
```{r}
# Table 2
TSresults <- vector("list", length(arrivalTimes))
for(i in 1:length(arrivalTimes)){
  TSresults[[i]] <- computeTable2(nls_results[[i]])
}
```
